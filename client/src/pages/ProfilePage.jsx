...
import {
  ...
  FiEdit2, FiPlus, FiTrash2, FiCheck, FiPackage,
  FiHeart, FiStar, FiLogOut
} from 'react-icons/fi';
...
export default function ProfilePage() {
  ...
  const stats = [
    ...
    { label: 'Total Spent',   value: `₦${orders.reduce((s,o) => s + (o.total||0), 0).toLocaleString()}`, icon: FiStar },
      ...
      <div className="pro-header">
        <div className="pro-header-inner">
          ...
          <div className="pro-header-actions">
            {user.role === 'admin' && (
              <Link to="/admin" className="pro-admin-btn">Admin Panel →</Link>
            )}
      ...
      <form className="pro-form" onSubmit={save}>
        ...
        <div className="pro-account-info">
          {[
            ...
            { label: 'Member Since', value: new Date(user.createdAt).toLocaleDateString('en-NG', { month: 'long', year: 'numeric' }) },
            { label: 'Account Type', value: user.role?.charAt(0).toUpperCase() + user.role?.slice(1) },
            { label: 'Status',       value: user.isActive ? 'Active' : 'Inactive', green: user.isActive },
          ].map(({ label, value, green }) => (
            <div key={label} className="pro-info-card">
              <span className="pro-info-label">{label}</span>
              <span className={`pro-info-val ${green ? 'green' : ''}`}>{value}</span>
            </div>
          ))}
        </div>
...
function AddressesTab({ user }) {
  ...
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({
    label: 'Home', street: '', city: '', state: '', zipCode: '', country: 'Nigeria', isDefault: false
  });
  ...
  const openNew = () => {
    setForm({ label: 'Home', street: '', city: '', state: '', zipCode: '', country: 'Nigeria', isDefault: false });
    setEditIndex(null);
    setShowForm(true);
  };
    ...
    <div className="pro-tab-content">
      ...
      <div className="pro-addr-grid">
        {addresses.map((addr, i) => (
          <div key={i} className={`pro-addr-card ${addr.isDefault ? 'default' : ''}`}>
            {addr.isDefault && (
            ...
            <div className="pro-addr-lines">
              ...
              <p>{addr.zipCode && `${addr.zipCode}, `}{addr.country}</p>
            </div>
            <div className="pro-addr-actions">
              ...
              {!addr.isDefault && (
                <button className="pro-addr-btn default-btn" onClick={() => setDefault(i)}>
          ...
          <form className="pro-form" onSubmit={saveAddress}>
            <div className="pro-form-grid">
              <div className="pro-field">
                ...
                <select className="pro-input" value={form.label} onChange={e => set('label', e.target.value)}>
              ...
              <div className="pro-field">
                ...
                <input className="pro-input" value={form.zipCode}
            ...
            <div className="pro-addr-form-btns">
              <button type="button" className="pro-btn-cancel" onClick={() => setShowForm(false)}>
                Cancel
              </button>
              <button type="submit" className="pro-save-btn" disabled={saving}>
                {saving ? 'Saving...' : editIndex !== null ? 'Update Address' : 'Add Address'}
              </button>
            </div>
          </form>
        </div>
  ...
  return (
    <div className="pro-tab-content">
      ...
      <form className="pro-form" onSubmit={save} style={{ maxWidth: '480px' }}>
          ...
          <div key={key} className="pro-field" style={{ marginBottom: '18px' }}>
            ...
            <div className="pro-pwd-wrap">
              <input
                ...
                required
              />
              <button type="button" className="pro-pwd-toggle"
                ...
                <div className="pro-pwd-bar">
                  {[1,2,3,4].map(i => (
                    <div
                      ...
                      style={{ background: i <= pwdStrength ? strengthColor : '#e8e8e8' }}
                    />
                  ))}
                </div>
                <span style={{ color: strengthColor, fontSize: '12px', fontWeight: 600 }}>
                  {strengthLabel}
                </span>
              </div>
            )}
        ...
        <button type="submit" className="pro-save-btn" disabled={saving} style={{ marginTop: '24px' }}>
  ...
  return (
    <div className="pro-tab-content">
      ...
      <div className="pro-wa-benefits">
        {[
          { icon: '📦', title: 'Order Confirmations',   desc: 'Instant confirmation when your order is placed' },
          { icon: '🚚', title: 'Shipping Updates',       desc: 'Track your delivery in real time' },
          { icon: '✅', title: 'Delivery Alerts',        desc: 'Know the moment your order arrives' },
          { icon: '🎁', title: 'Exclusive Offers',        desc: 'Get deals and promotions first' },
        ].map(({ icon, title, desc }) => (
        ...
        <div className="pro-field">
          ...
          <div className="pro-wa-input-wrap">
            ...
            <input
              ...
              onChange={e => setNumber('+234' + e.target.value.replace(/\D/g, ''))}
              placeholder="8012345678"
              type="tel"
            />
          </div>
          <span className="pro-input-note">Enter number without country code</span>
        </div>

        <button type="submit" className="pro-save-btn" disabled={saving} style={{ marginTop: '20px' }}>
