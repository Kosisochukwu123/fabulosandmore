...
const FAQ = [
  ...
  { q: 'How long does bulk delivery take?',    a: 'Standard bulk delivery takes 5–10 business days within Nigeria. Express options are available.' },
  ...
  { q: 'Do you offer credit terms?',           a: 'Verified businesses with 3+ orders may qualify for 30-day credit terms. Contact us to apply.' },
  { q: 'What payment methods are accepted?',   a: 'We accept bank transfers, cheques, and card payments. 50% deposit is required upfront.' },
...
export default function BulkOrderPage() {
  ...
  /* Build all contact links from settings */
  const waRaw    = business?.whatsapp || '';
  const waNumber = waRaw.replace(/[^0-9]/g, '');
  const phone    = business?.phone    || waRaw;
  const email    = business?.supportEmail || business?.email || '';
  ...
  /* WhatsApp message built from form state */
  ...
  const waFormLink   = waBase !== '#' ? `${waBase}?text=${whatsappMsg}` : '#';
  const waDirectLink = waBase !== '#' ? `${waBase}?text=${encodeURIComponent('Hi! I want to discuss a bulk order.')}` : '#';
  const waBottomLink = waBase !== '#' ? waBase : '#';
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitted(true);
    toast.success("Inquiry received! We'll contact you within 2 hours.");
    setSubmitting(false);
  };
        ...
        <div className="bulk-hero-inner">
          <div className="bulk-hero-text">
            ...
            <div className="bulk-hero-btns">
              ...
              <a href={waFormLink} target="_blank" rel="noreferrer" className="bulk-hero-btn-wa">
                <FaWhatsapp /> WhatsApp Us
              </a>
            </div>
            <div className="bulk-hero-stats">
              {[['500+','Happy Businesses'], ['40%','Max Discount'], ['2hrs','Response Time']].map(([val, label]) => (
                ...
                {submitted ? (
                  <div className="bulk-success">
                    ...
                    <div className="bulk-success-actions">
                      <a href={waFormLink} target="_blank" rel="noreferrer" className="bulk-wa-direct-btn">
                        <FaWhatsapp /> Chat Now for Faster Response
                      </a>
                      <button className="bulk-reset-btn"
                    ...
                    <form className="bulk-form" onSubmit={handleSubmit}>
                      <div className="bulk-form-row">
                        <div className="bulk-field">
                          <label className="bulk-label"><FiUsers size={13} /> Your Name *</label>
                          <input className="bulk-input" value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. Chioma Okafor" required />
                        </div>
                        <div className="bulk-field">
                          <label className="bulk-label"><FiUsers size={13} /> Company / Business</label>
                          <input className="bulk-input" value={form.company} onChange={e => set('company', e.target.value)} placeholder="e.g. Sunrise Restaurant Ltd" />
                        </div>
                      </div>
                      <div className="bulk-form-row">
                        <div className="bulk-field">
                          <label className="bulk-label"><FiMail size={13} /> Email Address *</label>
                          <input className="bulk-input" type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="your@email.com" required />
                        </div>
                        <div className="bulk-field">
                          <label className="bulk-label"><FiPhone size={13} /> Phone / WhatsApp *</label>
                          <input className="bulk-input" type="tel" value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="e.g. 08012345678" required />
                        </div>
                      </div>
                      <div className="bulk-form-row">
                        <div className="bulk-field">
                          <label className="bulk-label">Product Category</label>
                          <select className="bulk-input" value={form.category} onChange={e => set('category', e.target.value)}>
                        ...
                        <div className="bulk-field">
                          ...
                          <select className="bulk-input" value={form.quantity} onChange={e => set('quantity', e.target.value)}>
                      ...
                      <div className="bulk-field">
                        <label className="bulk-label"><FiPackage size={13} /> Products Needed *</label>
                        <textarea className="bulk-input bulk-textarea" value={form.products} onChange={e => set('products', e.target.value)}
                          placeholder="e.g. 200x non-stick frying pans 28cm, 100x wooden spatula sets..." required />
                      </div>
                      <div className="bulk-field">
                        <label className="bulk-label"><FiDollarSign size={13} /> Estimated Budget (₦)</label>
                        <input className="bulk-input" value={form.budget} onChange={e => set('budget', e.target.value)} placeholder="e.g. ₦500,000 – ₦1,000,000" />
                      </div>
                      <div className="bulk-field">
                        <label className="bulk-label"><FiMessageSquare size={13} /> Additional Notes</label>
                        <textarea className="bulk-input bulk-textarea-sm" value={form.note} onChange={e => set('note', e.target.value)}
                      ...
                      <div className="bulk-or-divider"><span>or</span></div>
                      <a href={waFormLink} target="_blank" rel="noreferrer" className="bulk-wa-alt-btn">
            ...
            <aside className="bulk-sidebar">
              <div className="bulk-contact-card">
                ...
                <div className="bulk-contact-items">
                  {phone && (
                    <a href={`tel:${phone.replace(/\s/g,'')}`} className="bulk-contact-item">
                      <div className="bulk-contact-icon phone"><FiPhone /></div>
                      <div>
                        <div className="bulk-contact-label">Call Us</div>
                        <div className="bulk-contact-val">{phone}</div>
                      </div>
                    </a>
                  )}
                  {waNumber && (
                    <a href={waDirectLink} target="_blank" rel="noreferrer" className="bulk-contact-item">
                      <div className="bulk-contact-icon wa"><FaWhatsapp /></div>
                      <div>
                        <div className="bulk-contact-label">WhatsApp</div>
                        <div className="bulk-contact-val">{business?.whatsapp || waNumber}</div>
                      </div>
                    </a>
                  )}
                  {email && (
                    <a href={`mailto:${email}`} className="bulk-contact-item">
                      <div className="bulk-contact-icon email"><FiMail /></div>
                      <div>
                        <div className="bulk-contact-label">Email</div>
                        <div className="bulk-contact-val">{email}</div>
                      </div>
                    </a>
                  )}
                </div>
              ...
              <div className="bulk-catalog-card">
                ...
                <Link to="/catalog" className="bulk-catalog-btn">Browse Full Catalog →</Link>
              </div>
            </aside>
          </div>
        </div>
      ...
      <section className="bulk-faq">
        <div className="bulk-container">
          ...
          <div className="bulk-faq-list">
            {FAQ.map((item, i) => (
              <div key={i} className={`bulk-faq-item ${openFaq === i ? 'open' : ''}`}>
                <button className="bulk-faq-q" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                  <span>{item.q}</span>
                  <span className="bulk-faq-arrow">{openFaq === i ? '−' : '+'}</span>
                </button>
                {openFaq === i && <div className="bulk-faq-a">{item.a}</div>}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bulk-container">
          <div className="bulk-bottom-cta-inner">
            ...
            <div className="bulk-bottom-cta-btns">
              <a href="#inquiry-form" className="bulk-hero-btn-primary">Get a Quote Now</a>
              <a href={waBottomLink} target="_blank" rel="noreferrer" className="bulk-hero-btn-wa">
