...
export default function Navbar() {
  ...
  const isActive = (path) => {
    ...
    return location.search.includes(
      encodeURIComponent(path.split("=")[1] || ""),
    );
  };
    ...
    <nav className={`nav-root ${scrolled ? "nav-scrolled" : ""}`}>
          ...
          {user ? (
            <div className="nav-user-wrap" ref={dropRef}>
              ...
              >
                ...
                <FiChevronDown
                  className={`nav-avatar-chevron ${dropOpen ? "open" : ""}`}
                />
              </button>
      ...
      <div className={`nav-mobile-menu ${menuOpen ? "open" : ""}`}>
        <div className="nav-mobile-links">
          ...
          <Link
            to="/catalog"
            className="nav-mobile-link nav-mobile-link-gold"
            onClick={() => setMenuOpen(false)}
          >
            View All Products →
          </Link>
          ...
          {user ? (
            <>
              ...
              <div className="nav-mobile-user-links">
                <Link
                  to="/profile"
                  className="nav-mobile-user-link"
                  onClick={() => setMenuOpen(false)}
                >
                ...
                <button
                  className="nav-mobile-user-link nav-mobile-logout"
                  onClick={() => {
                    logout();
                    setMenuOpen(false);
                  }}
